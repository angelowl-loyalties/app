import { AddIcon, CheckCircleIcon, DeleteIcon, EditIcon, MinusIcon } from '@chakra-ui/icons';
import {
    HStack,
    IconButton,
    Table,
...
export default function Home() {
    ...
    useEffect(() => {
        ...
        axios.get(`https://itsag1t2.com/campaign`, { headers: { Authorization: session.id } })
            .then((response) => {
                setCampaigns(response.data.data);
                axios.get(`https://itsag1t2.com/exclusion`, { headers: { Authorization: session.id } })
                    .then((response) => {
                                    ...
                                    {campaigns.map((campaign) => {
                                        return (
                                            <Tr key={campaign.id}>
                                                ...
                                                <Td fontSize="x-small">{campaign.base_reward ? <CheckCircleIcon color="green.400" /> : <MinusIcon color="red.400" />}</Td>
                                                <Td fontSize="x-small">{campaign.foreign_currency ? <CheckCircleIcon color="green.400" /> : <MinusIcon color="red.400" />}</Td>
                                                <Td fontSize="x-small"><IconButton variant="ghost" size="xs" icon={<EditIcon color="blue" />} /><IconButton size="xs" variant="ghost" icon={<DeleteIcon color="red" />} /></Td>
                                            </Tr>
                                        );
                                    })}
                                </Tbody>
